...
@Component({
  selector: 'app-addeval',
  templateUrl: './addeval.component.html',
  styleUrls: ['./addeval.component.css']
})
export class AddevalComponent implements OnInit {
  ...
  constructor(
    private evalformationService: EvalformationService,
  ...
  deleteEvaluation(id: number): void {
    ...
    this.evalformationService.deleteEvaluation(id).subscribe(() => {
  ...
  openModal(isEditing: boolean, evalformation?: any): void {
    ...
    modalRef.componentInstance.evalformation = evalformation || {};
  ...
  openConfirmationModal(): void {
    const modalRef: NgbModalRef = this.modalService.open(this.confirmationModal);
    setTimeout(() => {
      modalRef.close();
    }, 3000); // Ferme le modal automatiquement après 3 secondes
  }
}
